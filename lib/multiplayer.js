...
var i_pl = 1, i_rooms = 1,
	...
	allowedToEnter = {}, // успел первым или нет
	sentences = require('./sentences.js').sentences;
...
function handleJoinGame(socket) {
	...
	socket.on('game pair join', function(roomCode) {
		if (!io.sockets.adapter.rooms[roomCode]){
			io.to(this.id).emit('exit room', 'No room with such code found...');
			return;
		} 
...
function handleChangeSentence(socket) {
	socket.on('show new sentence', function(){
		if (allowedToEnter[currentRoom[this.id]]){
			allowedToEnter[currentRoom[this.id]] = false;
...
function createSentence(socket) {
	var	serverSentence = sentences[getRandomInt(0, sentences.length - 1)];
	
	socket.on('allow to enter sentence', function(){
		allowedToEnter[currentRoom[socket.id]] = true;
	});
